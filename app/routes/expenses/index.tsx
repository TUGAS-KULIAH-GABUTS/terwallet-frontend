...
import { IExpenseModel } from '~/models/expenseModel'
...
export let loader: LoaderFunction = async ({ params, request }) => {
  ...
  try {
    const result = await API.getTableData({
      session: session,
      url: CONFIG.baseUrlApi + '/expenses',
      pagination: true,
      page: +page || 0,
      size: +size || 10,
      filters: {
  ...
  try {
    ...
    if (request.method == 'DELETE') {
      reponse = await API.delete(
        session,
        CONFIG.baseUrlApi + `/expenses?expenseId=${formData.get('expenseId')}`
      )
    }
...
export default function Index(): ReactElement {
  ...
  const [modalData, setModalData] = useState<IExpenseModel>()
  ...
  const submitDeleteData = async (e: React.FormEvent<HTMLFormElement>) => {
    submit(e.currentTarget, { method: 'delete', action: `/expense` })
  ...
  const header: TableHeader[] = [
    {
      title: 'No',
      data: (data: IExpenseModel, index: number): ReactElement => (
    ...
    },
    {
      title: 'Nama',
      data: (data: IExpenseModel, index: number): ReactElement => (
        <td key={index + 'incomeName'} className="md:px-6 md:py-3">
          {data.expenseName}
        </td>
      )
    },
      ...
      data: (data: IExpenseModel, index: number): ReactElement => (
        <td key={index + 'action'} className="md:px-6 md:py-3">
          ...
          <div className="hidden md:block w-64">
            ...
            &nbsp;
            <Link to={`/expenses/edit/${data.expenseId}`}>
          ...
          <div className="block md:hidden relative">
            ...
            >
              <ul className="py-1" aria-labelledby={`dropdownButton-${index}`}>
                <li>
                  <Link
                    to={`/expenses/edit/${data.expenseId}`}
    ...
    <div className="">
      ...
      >
        <Form method="delete" onSubmit={submitDeleteData}>
          <input className="hidden" name="expenseId" value={modalData?.expenseId} />
          Anda yakin akan menghapus <strong>{modalData?.expenseName}</strong>
