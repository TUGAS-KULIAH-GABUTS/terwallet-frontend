...
export let loader: LoaderFunction = async ({ params, request }) => {
  ...
  try {
    const result = await API.get(
      session,
      CONFIG.baseUrlApi + `/incomes/detail/${params.incomeId}`
    )
    return {
      incomes: result,
      session: session,
  ...
  try {
    if (request.method == 'PATCH') {
      const payload: IAdminCreateRequestModel | any = {
        incomeName: formData.get('incomeName'),
        incomeNominal: formData.get('incomeNominal')
      }

      await API.post(session, CONFIG.baseUrlApi + '/incomes', payload)
...
export default function Index() {
  ...
  const incomes: IIncomeModel = loader?.incomes
  ...
  const submitData = async (e: React.FormEvent<HTMLFormElement>) => {
    submit(e.currentTarget, {
      method: 'patch',
      action: `/incomes/edit/${incomes.incomeId}`
    })
  }
  ...
  return (
    <div className="">
      ...
      <Form method={'patch'} onSubmit={submitData} className="bg-white rounded-xl p-10">
        <input hidden name="incomeId" value={incomes.incomeId} />
        <div className="w-full md:mr-2">
          <div className="my-6">
            ...
            <input
              ...
              required
              placeholder="nama..."
              defaultValue={incomes.incomeName}
            />
          </div>
          <div className="my-6">
            ...
            <input
              ...
              required
              placeholder="nama..."
              defaultValue={incomes.incomeNominal}
            />
          </div>
        </div>
