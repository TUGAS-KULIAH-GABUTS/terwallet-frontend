  ...
  try {
    ...
    if (request.method == 'DELETE') {
      reponse = await API.delete(
        session,
        CONFIG.baseUrlApi + `/incomes?incomeId=${formData.get('incomeId')}`
      )
    }
...
export default function Index(): ReactElement {
  ...
  const [modalData, setModalData] = useState<IIncomeModel>()
  ...
  const submitDeleteData = async (e: React.FormEvent<HTMLFormElement>) => {
    submit(e.currentTarget, { method: 'delete', action: `/incomes` })
  ...
  const header: TableHeader[] = [
    {
      title: 'No',
      data: (data: IIncomeModel, index: number): ReactElement => (
      ...
      data: (data: IIncomeModel, index: number): ReactElement => (
        <td key={index + 'action'} className="md:px-6 md:py-3">
          ...
          <div className="hidden md:block w-64">
            ...
            &nbsp;
            <Link to={`/incomes/edit/${data.incomeId}`}>
  ...
  return (
    <div className="">
      <Breadcrumb title="Incomes" navigation={navigation} />
      ...
      >
        <Form method="delete" onSubmit={submitDeleteData}>
          <input className="hidden" name="incomeId" value={modalData?.incomeId} />
          Anda yakin akan menghapus <strong>{modalData?.incomeName}</strong>
